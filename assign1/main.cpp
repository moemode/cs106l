...
struct Course {
  ...
  // Constructor that initializes a Course object from an input stream
  Course(std::istream &is) {
    std::string line;
    if (std::getline(is, line)) {
      std::istringstream iss(line);
      std::getline(iss, title, ',');
      std::getline(iss, number_of_units, ',');
      std::getline(iss, quarter);
    }
  }

  Course(const std::string &t, const std::string &n, const std::string &q)
      : title(t), number_of_units(n), quarter(q) {}
};
 ...
 */
std::string parse_csv(std::string filename, std::vector<Course> &courses) {
  ...
  while (std::getline(file, line)) {
    ...
    if (fields.size() == 3) {
      courses.emplace_back(fields[0], fields[1], fields[2]);
    }
  }
  return header;
}
 ...
 * courses.
 */
void write_courses_offered(std::string header, std::vector<Course> &all_courses) {
  std::ofstream file(COURSES_OFFERED_PATH);
  file << header << std::endl;

  auto it = std::remove_if(all_courses.begin(), all_courses.end(), [&](const Course& course) {
 ...
 */
void write_courses_not_offered(std::string header, std::vector<Course> &unlisted_courses) {
...
int main() {
  ...
  std::string header = parse_csv("courses.csv", courses);
  print_courses(courses);

  write_courses_offered(header, courses);
  write_courses_not_offered(header, courses);
